...
async def get_recipe_img(request: Request, recipe_id: UUID4, file_name: ImageType = ImageType.original):
    """Takes in a recipe id, returns the static image."""
    recipe_image = Recipe.directory_from_id(recipe_id, create=False).joinpath("images", file_name.value)
    return cached_file_response(recipe_image, request)
...
async def get_recipe_timeline_event_img(
    ...
    timeline_event_image = RecipeTimelineEventOut.image_dir_from_id(
        recipe_id, timeline_event_id, create=False
    ).joinpath(file_name.value)
    return cached_file_response(timeline_event_image, request)


@router.get("/{recipe_id}/assets/{file_name}")
async def get_recipe_asset(recipe_id: UUID4, file_name: str):
    """Returns a recipe asset"""
    asset_dir = Recipe.directory_from_id(recipe_id, create=False).joinpath("assets")
